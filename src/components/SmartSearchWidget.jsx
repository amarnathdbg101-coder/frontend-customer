...
export function SmartSearchWidget({ onSelectProduct, onClose }) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("shopme_recent_searches");
    if (saved) {
  ...
  const handleSearchChange = async (text) => {
    ...
    setIsLoading(true);
    try {
      // 1. Instant local/API product search
      const res = await apiClient.get(`/products?search=${encodeURIComponent(text)}&limit=8`);
  ...
  const handleSelect = (product) => {
    ...
    localStorage.setItem("shopme_recent_searches", JSON.stringify(updated));
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">SKU: {prod.sku || "N/A"} • Category: {prod.category || "General"}</p>
                </div>
              </div>
